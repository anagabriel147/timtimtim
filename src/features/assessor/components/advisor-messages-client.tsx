...
import { AdvisorTopbar } from './advisor-topbar'
import {
  ADVISOR_CHAT_COMPOSER,
  ADVISOR_CHAT_FILTERS,
  ADVISOR_CONVERSATIONS,
  type AdvisorChatMessage,
  type AdvisorConversation,
} from '../data/advisor-chat-data'

function nowTime() {
  return new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
}

function initialConversations(): AdvisorConversation[] {
  return ADVISOR_CONVERSATIONS.map((c) => ({ ...c, messages: c.messages.map((m) => ({ ...m })) }))
}

export function AdvisorMessagesClient() {
  const [conversations, setConversations] = useState<AdvisorConversation[]>(initialConversations)
  const [activeId, setActiveId] = useState<string>(
    ADVISOR_CONVERSATIONS.find((c) => c.active)?.id ?? ADVISOR_CONVERSATIONS[0].id,
  )
  const [filter, setFilter] = useState<(typeof ADVISOR_CHAT_FILTERS)[number]>('Todos')
  ...
  const matchesFilter = (tag?: string) => {
    if (filter === 'Contratação') return tag === 'Contratação'
    return true
  }
  ...
  const handleSend = () => {
    ...
    if (!trimmed) return
    const message: AdvisorChatMessage = {
      id: `s-${Date.now()}`,
      from: 'advisor',
      time: nowTime(),
      text: trimmed,
    ...
    setConversations((prev) =>
      prev.map((c) =>
        c.id === active.id
          ? { ...c, messages: [...c.messages, message], preview: `Você: ${trimmed}`, timestamp: message.time }
          : c,
      ),
    )
    setDraft('')
  ...
  return (
    <div className="flex h-svh flex-col overflow-hidden bg-background">
      <AdvisorTopbar active="Mensagens" onUnavailable={(l) => setToast(`${l} em breve.`)} />
          ...
          <div className="flex items-center justify-between px-5 pb-3 pt-5">
            ...
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                onClick={() => setToast('Nova conversa: selecione um casal para iniciar.')}
                className="grid size-8 place-items-center rounded-lg text-muted-foreground transition-colors hover:bg-secondary/60 hover:text-foreground"
              >
          ...
          <div className="flex items-center justify-between gap-4 border-b border-border px-6 py-3">
            ...
            <div className="flex shrink-0 items-center gap-2">
              <button
                ...
                className="hidden items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm text-foreground transition-colors hover:bg-secondary/60 sm:flex"
              >
          ...
          {active.banner && bannerOpen && (
            <div className="flex flex-wrap items-center gap-3 border-b border-border bg-secondary/30 px-6 py-3">
              ...
              <div className="ml-auto flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setToast('Abrindo o painel do casal.')}
            ...
            <div className="mx-auto max-w-3xl">
              <div className="flex items-end gap-2 rounded-2xl border border-primary/30 bg-card px-3 py-2">
                ...
                <textarea
                  ...
                  rows={1}
                  placeholder={`${ADVISOR_CHAT_COMPOSER.placeholderPrefix} ${active.shortName}...`}
              ...
              <div className="mt-2 flex items-center justify-between px-1 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  ...
                  {ADVISOR_CHAT_COMPOSER.secureNote}
                </span>
                <span className="hidden sm:block">{ADVISOR_CHAT_COMPOSER.hint}</span>
              </div>
            </div>
          </div>
        </section>
...
function MessageBubble({
  ...
  return (
    <div className={cn('flex items-end gap-2', isAdvisor ? 'justify-end' : 'justify-start')}>
      {!isAdvisor && <Avatar convo={convo} size={32} />}
