...
export function PaymentModal({
  ...
  const cardValid = useMemo(
    () => number.replace(/\s/g, '').length >= 16 && name.trim().length > 2 && expiry.length >= 4 && cvv.length >= 3,
        ...
        {status === 'approved' ? (
          ...
          <>
            ...
            <div className="flex items-center justify-between px-6 py-4">
              ...
              <span className="font-display text-2xl font-bold text-primary">{payment.value}</span>
            </div>
              ...
              {method === 'pix' ? (
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Expira em <span className="font-semibold text-primary">{PIX.expiresIn}</span>
                  </p>
                  <div className="mt-4 flex w-full items-center gap-2 rounded-lg border border-border bg-background/60 p-2">
                ...
                <div className="space-y-3">
                  <Field label="Número do cartão">
                    <input
                      ...
                      className="w-full rounded-lg border border-border bg-background/60 px-3 py-2.5 text-sm text-foreground outline-none placeholder:text-muted-foreground focus:border-primary"
                    />
                  </Field>
                  ...
                  <Field label="Parcelas">
                    <select
                      ...
                      className="w-full rounded-lg border border-border bg-background/60 px-3 py-2.5 text-sm text-foreground outline-none focus:border-primary"
                    >
                      {INSTALLMENTS.map((i) => (
...
function ApprovedView({
  ...
  return (
    <div className="flex flex-col items-center px-6 py-12 text-center">
      ...
      <h2 className="mt-5 font-display text-2xl font-bold text-foreground">Pagamento aprovado!</h2>
      <p className="mt-2 max-w-sm text-sm text-muted-foreground">
        O contrato com <span className="font-medium text-foreground">{payment.vendor}</span> foi
        firmado e o valor de{' '}
        <span className="font-semibold text-primary">{payment.value}</span> está retido em escrow
        {method === 'pix' ? ' (Pix confirmado)' : ' (cartão aprovado)'}.
      </p>
      <div className="mt-6 flex items-center gap-2 rounded-full border border-primary/40 bg-primary/10 px-4 py-2 text-sm text-primary">
        <Loader2 className="size-4 animate-spin" />
